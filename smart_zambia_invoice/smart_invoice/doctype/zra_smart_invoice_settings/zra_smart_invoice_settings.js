...
frappe.ui.form.on('ZRA Smart Invoice Settings', {
  refresh(frm) {
      ...
      if (!frm.is_new() && frm.doc.is_active) {
          ...
          actions.forEach(action => {
              frm.add_custom_button(action.label, () => {
                  frappe.call({
  ...
  sandbox(frm) {
      ...
      const serverUrl = sandboxFieldValue === 1 
...
function switch_environment(frm, env) {
  ...
  frm.set_value('server_url', isSandbox ? 'http://localhost:8080/zrasandboxvsdc' : 'http://kindatech.group:8080/zrasandboxvsdcs');
  frm.set_value('environment', isSandbox ? 'Sandbox' : 'Production');
  frm.set_value(isSandbox ? 'production_environment_' : 'sandboxtest_environment_', 0);
}
