...
def on_successful_fetch_latest_items(frm, response):
    ...
    items_updated = 0  # Track if any updates are made
    ...
    for item in response.get('data', {}).get('itemList', []):
        ...
        frappe.log_error(str(item), "ZRA Item Debug")
        ...
        # Ensure the classification code is set correctly
        classification_code = item.get("itemClsCd", "").strip()  # Ensuring it's not None
        if not classification_code:
            classification_code = "DEFAULT_CLASSIFICATION"  # Set a fallback if missing

        doc.custom_zra_item_classification_code = classification_code  # Assign the value

        doc.item_group = "All Item Groups"
        doc.custom_zra_item_code = item_code  
        ...
        doc.standard_rate = float(item.get("dftPrc", frm.get("valuation_rate", 0)) or 0)
        doc.rrp = float(item.get("rrp", 0) or 0)
        doc.safety_quantity = float(item.get("sftyQty", "0") or 0)

        doc.custom_zra_country_of_origin = code_name
        doc.custom_zra_packaging_unit = packaging_unit
        doc.custom_zra_unit_quantity_code = unit_quantity
        doc.custom_zra_tax_type = item.get("vatCatCd", frm.get("custom_zra_tax_type", "DefaultTax"))
        doc.batch_no = item.get("btchNo")
        doc.custom_zra_product_type_code = product_type
        ...
        try:
            ...
            frappe.log_error(f"Error saving item {item_code}: {str(e)}", "ZRA Item Fetch Error")

    # Show message only if no updates were made
    if items_updated == 0:
        frappe.msgprint(__('Nothing to update.'))
    else:
        frappe.msgprint(_(f'{items_updated} items updated successfully.'))
